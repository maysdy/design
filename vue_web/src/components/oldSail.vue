        ...
        <el-card shadow="always">
          <el-form label-position="top" label-width="80px" :model="submitGoodsForm" :rules="rules"
            ...
            <el-col :span="18" :offset="2">
              <el-form-item label="农产品图" enctype="multipart/form-data">
                <!--图片上传组件-->
                <el-upload
...
<!--                <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>-->
            ...
            <el-col :span="3" :offset="2" style="margin-top: 20px;">
              <el-form-item label="成熟日期" prop="date">
                <el-date-picker
                  v-model="submitGoodsForm.date"
                  type="date"
                  placeholder="选择日期"
                style="width: 160px">
                </el-date-picker>
            ...
            <el-col :span="4" :offset="2" style="margin-top: 20px;">
              <el-form-item label="农产品类型" prop="type">
              <el-select v-model="submitGoodsForm.type" placeholder="请选择类型">
                <el-option
           ...
           <!-- <el-col :span="9" :offset="2" style="margin-top: 20px;">
              <el-form-item label="商品数量（个）" prop="goodsLevel">
                <el-input v-model="submitGoodsForm.goodsLevel"></el-input>
              </el-form-item>
            </el-col>-->
            <el-col :span="9" :offset="2" style="margin-top: 20px;">
              <el-form-item label="产地" prop="origin">
                <el-input v-model="submitGoodsForm.origin"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="9" :offset="2" style="margin-top: 20px;">
              <el-form-item label="农产品介绍" prop="dscrip">
                <el-input
                  type="textarea"
  ...
  export default {
    ...
    data() {
      let checkPrice = (rule, value, callback) => {
        ...
        if (value === "") {
          return callback(new Error('农产品价格不能为空'));
        } else {
      ...
      let checkGoodsLevel = (rule, value, callback) => {
        let numberRex = /^[1-9]\d*$/;
        if (value === "") {
          return callback(new Error('农产品数量不能为空'));
        } else {
          if (!numberRex.test(value)) {
            callback(new Error('请输入正整数'));
          } else {
            if (value < 1 || value > 999) {
              callback(new Error('必须在 1 - 999 之间'));
            } else {
              callback();
            }
          }
      ...
      return {
        ...
        rules: {
          name: [
            {validator: checkString, trigger: 'blur',required : true}
          ],
          price: [
            {validator: checkPrice, trigger: 'blur',required : true}
          ],
          goodsLevel: [
            {validator: checkGoodsLevel, trigger: 'blur',required : true}
          ],
          origin: [
            {validator: checkString, trigger: 'blur',required : true}
          ],
    ...
    mounted() {
      ...
      $.get("http://localhost:8083/goods/getRandomGoodsId.do",function (data) {
        ...
        self.submitPlanForm.goodsId = data.goodsId;
        //设置上传图片路径，需要一个商品ID
        self.action = "http://localhost:8083/upload/uploadFile.do?goodsId=" + data.goodsId;
      },"json");
      $.get("http://localhost:8083/plan/getRandomPlanId.do",function (data) {
        self.submitPlanForm.planId = data.planId;
        // this.submitPlanForm.planId = data.planId;
        console.log("planId:"+self.submitPlanForm.planId);
      },"json");
    },
    methods: {
        ...
        if(this.submitFlag){
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.submitPlanForm.name = this.submitGoodsForm.name;
              let jsonMsg1 = JSON.stringify(this.submitPlanForm);
              $.post("http://localhost:8083/plan/createPlan.do",jsonMsg1,function (data) {
                // if (data.code === 1) {
                //   self.dialogValue = "保存成功";
                // } else {
                //   self.dialogValue = "保存失败，错误码：" + data.code;
