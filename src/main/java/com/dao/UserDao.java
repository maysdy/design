...
public interface UserDao {
    ...
    int findCount();
    //
    @Select("select money from users where user_id = #{userId}")
    Double findMoneyById(@Param("userId") String userId);
    //
    ...
    /*修改功能*/
    //根据ID修改用户昵称、密码、性别
    @Update("update users set name = #{name},password = #{password},sex = #{sex},address = #{address},money = #{money}where user_id = #{userId}")
    int changeUserById(@Param("userId") String userId, @Param("name") String name, @Param("password") String password, @Param("sex") int sex,@Param("address") String address,@Param("money") Double money);
    //根据ID修改用户密码
    ...
    //支付扣钱
    @Update("update users set money = money - #{money} where user_id = #{userId}")
    int changeMoney(@Param("userId") String userId,@Param("money") Double money);

    /*添加功能*/
    //创建新用户
    @Insert("insert into users values(#{userId},#{name},#{username},#{password},#{sex},#{email},#{status},#{admin},#{address},#{money})")
