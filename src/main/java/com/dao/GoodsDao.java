...
public interface GoodsDao {
    /*添加功能*/
    //新建商品信息
    @Insert("insert into goods values(#{userId},#{goodsId},#{dscrip},#{price},#{goodsLevel},#{upStatus},#{useStatus},#{hot},#{name},#{origin},#{type},#{date})")
    int createGoods(Goods goods);

    /*修改功能*/
    //根据商品ID修改商品介绍信息
    @Update("update goods set dscrip = #{dscrip},date = #{date} ,price = #{price},origin = #{origin},type = #{type} where goods_id = #{goodsId}")
    int changeDescriById(@Param("goodsId") String goodsId, @Param("dscrip") String dscrip, @Param("date")Date date,@Param("price") Double price ,@Param("origin") String origin,@Param("type") int type);
    //根据商品ID修改商品价格信息
...
//    @Update("update goods set dscrip = #{dscrip}, price = #{price}, name = #{name},origin = #{origin},type = #{type},date = #{date}")
    //商品数量减1
    @Update("update goods set goods_level = goods_level+1 where goods_id = #{goodsId}")
    int reducebyId(String goodsId);
    ...
    int findCount();
    //查询卖家id
    @Select("select user_id from goods where goods_id = #{goodsId}")
    String selectUser(@Param("goodsId") String goodsId);
}
