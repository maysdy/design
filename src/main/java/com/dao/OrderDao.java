...
public interface OrderDao {
    /*添加功能*/
    //新增订单
    @Insert("insert into orders values(#{userId},#{goodsId},#{ordersStatus},#{price},#{payId},#{ordersId},#{adminStatus},#{time},#{solderId},#{name})")
    ...
    //根据用户ID查询订单集合
    @Select("select * from orders where user_id = #{userId} or solder_id = #{userId}")
