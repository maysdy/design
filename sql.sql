...
CREATE TABLE `feedbacks` (
  ...
  `feedbacks_status` int(11) DEFAULT NULL COMMENT '类型（1投诉、0评价、-1其他）',
...
CREATE TABLE `orders` (
  `user_id` varchar(50) DEFAULT NULL COMMENT '用户ID',
  `goods_id` varchar(50) DEFAULT NULL COMMENT '商品ID',
  `orders_status` int(11) DEFAULT NULL COMMENT '订单状态（0未支付、1待发货、2已发货、3完成、-1取消）',
  `price` double DEFAULT NULL COMMENT '商品价格',
  `pay_id` varchar(50) DEFAULT NULL COMMENT '支付ID',
  `orders_id` varchar(50) NOT NULL COMMENT '订单ID（主键、随机码）',
  `admin_status` int(11) DEFAULT NULL COMMENT '处理状态（1已处理、0未处理）',
  `time` varchar(30) DEFAULT NULL,
   `solder_id` varchar(50) DEFAULT NULL COMMENT '卖家id',
